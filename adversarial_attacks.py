...
# DATA - IMPAGE PREPROCESSING AND DATASET LOADING, PREPARING RAW IMAGE FILES FOR MODEL TO CORRECTLY USE
...
def compute_gradient(image, model, lossFunction, actualOutput):
    ...
    loss.backward()
    #now, the loss's gradient is calculated with respect to the pixels and is retruned in the next line 
    return image.grad.data
#overall, finding each pixel's contribution to a wrong model

def fgsmAttack(image, epsilon, gradient):
    #follows the FGSM formulation - the gradient shows which direction increases the loss, so FGSM moves the image in that firection 
